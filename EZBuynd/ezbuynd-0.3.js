(function (globe) {
    globe.EZB = (function () {
        var _logMissingProperty = function (d, key) {
        ...
        var _createTemplate = function (el) {
            ...
            for (var i = 0; i < baseElement.children.length; i++) {
                ...
                if (attr.hasIgnore) {
                    ...
                    var cTpl = _createTemplate(baseElement.children[i]);
            ...
            template.appendNew = function (el) {
        ...
        */
        var _createObserver = function (key, valueBinding, data, el, binding) {
            ...
            if (_.isUndefined(data[options.rootSetFunctionName])) {
                if (!data[options.restoreOptionsKey].objectOptions.hasSetter) {
            ...
            if (!data[options.restoreOptionsKey].objectOptions.hasGetter) {
        ...
        var _createDependencyTemplate = function (template, data, binding) {
            var t = {
                binding: binding,
                template: template
            };
        ...
        var _createArrayRestoreOptions = function (datasource, binding) {
            ...
            datasource.push = function () {
                ...
                if (elements.length === 0) {
                    ...
                    var result = datasource[options.restoreOptionsKey].arrayOptions.push.apply(datasource, elements);

                    for (var i = 0; i < datasource[options.arrayTemplates].length; i++) {
                        var t = datasource[options.arrayTemplates][i];
        ...
        */
        var _setDataSource = function (datasource, originalElement, binding) {
            ...
            var childBindings = function (parent, parentElement, bindingData) {
                var els = [];
                    ...
                    if (!_.isNull(children[i].dependencyKey)) {
                        ...
                        dt.creator = function (newData) {
                            ...
                            for (var k = 0; k < template.valueBindings.length; k++) {
                                var vbinding = template.valueBindings[k];
                ...
                for (var prop in datasource) {
                    ...
                    if (Object.prototype.hasOwnProperty.call(datasource, prop)) {
                        ...
                        if (_.isArray(datasource[prop])) {
                            datasource.unbound.push(datasource[prop]);
        ...
        * @param binding The root EZBuynd binding
        */
        var _createObjectRestoreOptions = function (datasource) {
            if (_.isUndefined(datasource[options.restoreOptionsKey])) {
